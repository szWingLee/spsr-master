...
def adaptive_sampler(pic_size, crop_dict, num_candidate_dict, dist_map, min_diff=False):
    ...
    for k, v in crop_dict.items():
        crop_size = (int(k), int(k))
        crop_res_dict[k] = [
            _get_adaptive_crop_indices(crop_region, crop_size, num_candidate_dict[k], dist_map, min_diff) for _ in
            range(v)]
    return crop_res_dict
...
def pyramid_sampler(pic_size, crop_dict):
    ...
    for i in range(1, len(sorted_key)):
        ...
        for j in range(len(split_list)):
            ...
            crop_res_dict[sorted_key[i]].extend(
                [_get_random_crop_indices(crop_region, crop_size) for _ in range(split_list[j])])
